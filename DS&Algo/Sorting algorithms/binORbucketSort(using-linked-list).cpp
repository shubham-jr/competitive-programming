...
void insertion(vector<node*>&bucket,int data)
{
  node*tmp=new node;
  tmp->data=data;
  tmp->next=nullptr;
...
void deletion(vector<node*>&bucket)
{
  int n=bucket.size();
  node*tmp=nullptr;
  fo(0,n)
...
void solve()
{
  ...
  vector<node*>bucket(maxi+1,nullptr);
  fo(0,n)
  insertion(bucket,v[i]);
  deletion(bucket);
}
