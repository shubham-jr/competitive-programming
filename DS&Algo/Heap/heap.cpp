...
void heap_insertion(vector<int>&v, int k) {
    v.pb(k);

    int n = v.size();

    int j = n - 1, i = (j & 1) ? j / 2 : (j / 2) - 1;
...
void heap_deletion(vector<int>&v, int n) {
    swap(v[0], v[n - 1]);
...
void solve()
{
    ...
    // Creation
    while (cin >> k) {
        heap_insertion(v, k); //Insertion
    }

    // Heap Sort
    fo(0, n)
    heap_deletion(v, n - i); //Deletion
