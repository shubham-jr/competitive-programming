...
vector<vector<ll>>dp(2,vector<ll>(1000001,-1));
// for pre-computation
...
ll ways(ll n,ll i)
{
  ...
  ll single=0,multi=0;
  if(i==0)
  multi+=(ways(n-1,1)%mod+4*ways(n-1,0)%mod)%mod; 
  else 
  single+=(2*ways(n-1,1)%mod+ways(n-1,0)%mod)%mod;
  return dp[i][n]=(multi+single);  
}
...
void solve()
{
  ll n;
  cin>>n;
  cout<<(ways(n,0)%mod+ways(n,1)%mod)%mod<<nline;
}
