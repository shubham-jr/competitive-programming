...
int maxPage(vector<int>&price,vector<int>&page,vector<vector<int>>&dp,int x,int n)
{
  if(x<0)
  return INT_MIN;
  if(x==0)
  ...
  return dp[x][n];  
  int tmp;
  tmp=max(page[n-1]+maxPage(price,page,dp,x-price[n-1],n-1),maxPage(price,page,dp,x,n-1));
  return dp[x][n]=tmp;
}
...
void solve()
{
  ...
  fo(0,n)
  cin>>page[i];
  cout<<maxPage(price,page,dp,x,n);
}
