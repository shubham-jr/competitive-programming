...
int ways(string &str1,string &str2,int i,int j,vector<vector<int>>&dp)
{
  ...
  return dp[i][j]=ways(str1,str2,i-1,j-1,dp);
  else
  {
    int add=ways(str1,str2,i,j-1,dp);

    int remove=ways(str1,str2,i-1,j,dp);

    int replace=ways(str1,str2,i-1,j-1,dp);
